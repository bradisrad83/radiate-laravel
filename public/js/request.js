...
void 0 === window.centovacast.loader && (window.centovacast.loader = {
    ...
    jq_loaded: function() {
        ...
        this.loaded = !0;
        var a = this;
        jQuery(document).ready(function() { a.ready = !0; for (var b in a.widget_definitions) "function" === typeof a.widget_definitions[b].init && a.widget_definitions[b].init(jQuery) })
    },
    check: function() {
        if ("undefined" === typeof jQuery) {
            var c = this;
            setTimeout(function() { c.check() }, 100);
            this.attempts++
        } else this.jq_loaded()
    },
    ...
    process_widget_elements: function(c, a, b, e) {
        ...
        return {
            ...
            get_property: function(a, b) { return this.widget_data[a] && this.widget_data[a][b] ? this.widget_data[a][b] : void 0 },
            get_element: function(a, b) { return this.widget_data[a] ? this.widget_data[a].hastype ? this.widget_data[a].$el[b] ? this.widget_data[a].$el[b] : jQuery() : this.widget_data[a].$el ? this.widget_data[a].$el : jQuery() : void 0 },
            set_element: function(a, b, c) { this.widget_data[a].hastype ? b && b.length && (this.widget_data[a].$el[b] = c) : this.widget_data[a].$el = c },
            set_property: function(a, b, c) {
                if (!this.widget_data[a]) return !1;
                this.widget_data[a][b] = c;
                return !0
            },
            each: function(a) { for (var b in this.widget_data) "string" === typeof b && a(b, this.widget_data[b]) },
    ...
    add: function(c, a, b) {
        this.widget_definitions[c] || (this.widget_definitions[c] = { define: b, init: a, ref: null });
        this.loaded && this.load_widget(c);
        this.ready && a(jQuery)
    },
    jq_get_jsonp: function(c, a, b) { return jQuery.ajax({ type: "GET", url: c, data: a, success: b, dataType: "jsonp" }) }
}, window.centovacast.loader.init());
window.centovacast.loader.add("request", function(c) { window.centovacast.request.run() }, function(c) {
    c("<link/>", { rel: "stylesheet", type: "text/css", href: window.centovacast.loader.url + "theme/widget_request.css" }).appendTo(c("body"));
    return window.centovacast.request = {
        ...
        get_form_fields: function(a) {
            ...
            b.$rbtn = d;
            var d = { artist: b.$rartist, title: b.$rtitle, email: b.$remail, sender: b.$rsender },
                f;
            for (f in d)
        ...
        submit: function(a) {
            ...
            if (2 > d.length) e.$rartist.css({ backgroundColor: "#faa" }).focus().select();
            else if (1 > f.length) e.$rtitle.css({ backgroundColor: "#faa" }).focus().select();
            else if (h.match(/^[\w\.%+\-]{1,64}@[A-Z0-9][A-Z0-9\.\-]{0,253}[A-Z0-9]\.[A-Z]{2,16}$/i)) {
                e.$rbtn.prop("disabled", !0);
                this.show_throbber(a);
                var l = this;
                c.getJSONP((this.settings.local ? "/" : window.centovacast.loader.url) + "external/rpc.php", { m: "request.submit", username: b.username, charset: b.charset, artist: d, title: f, sender: g, email: h, dedi: k, rid: a }, function(b) { b && l.handle_json(b, a) })
            } else e.$remail.css({ backgroundColor: "#faa" }).focus().select()
        },
        create_throbber: function() {
            var a = c("<div/>", {
                "class": "cc_request_throbber",
                css: { position: "absolute", zIndex: 250, textAlign: "center" },
        ...
        hide_throbber: function(a) { this.get_throbber(a).hide() },
        get_result_container: function(a) { return this.widgets.get_element(a, "result") },
        init_form: function(a) {
            ...
            var f = this;
            e.click(function() { f.submit(a) })
        },
        init_all: function() {
            var a = this;
            this.widgets.each(function(b) { a.init_form(b) })
        },
        run: function() {
            this.widgets = window.centovacast.loader.process_widget_elements(c(this.element_class), { username: "username", cs: "charset", mp: "mount" }, "cc_req", !0);
            this.init_all()
        }
    }
});